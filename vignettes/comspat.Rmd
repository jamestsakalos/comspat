---
title: "Getting started with 'comspat'"
...
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Getting started with 'comspat'}
...
```{r view-hypothetical-communities, fig.width = 7.1, fig.cap="**Fig. S2**. Spatial distribution of the patchy associated, patchy no interspecific association and completely random (i.e. the grid patchy_associated, grid_patchy_n_isc and grid_mixed_random, resp.) data sets."}
...
```{r view-frequency-combinations, fig.width = 7.1, fig.cap="**Fig. S4**. Barplot demonstrating the differences of the frequency distribution of species combinations among the contrasting types of patterns represented by the grid data sets. Each bar represents a specific species combination occurring within the smallest sample unit size."}
...
# Create the barplot (note that we are using log transformation so was can
# better view the different combinations)
par(mar = c(8, 4, 1, 1))
...
```{r view-transect-patterns, fig.width = 7.1, fig.cap="**Fig. S5**. Spatial distribution of the species and plant functional types along the length of the transect data sets."}
# View the species spatial distributions
...
# Plots the transect plant functional type data frame
plot(
  as.numeric(tran_grass_t$Species) ~ as.numeric(tran_grass_t$X),
     xaxt = "n", yaxt = "n", ann = FALSE, xlim = c(0, 500),
     pch = c(3, 4, 15:18)[tran_grass_t$Species],
     col = c(
       "red", "blue", "green", "pink", "orange", "black"
     )[tran_grass_t$Species]
)
axis(1, at = c(0, 500), labels = c(0, 500))
...
```{r view-transect-combinations, fig.width = 7.1, fig.cap="**Fig. S6**. Frequency distribution of the species and plant functional types (PFT)  within the transect data sets."}
# View the species frequency of occurrence
...
```{r view-spatial-scaling, fig.cap="**Fig. S7**. Example of spatial scaling using exponentially increaseing of in secondary sampling unit sizes."}
...
```{r load-ms-data, echo  =FALSE}
load("ms_results.rdata")
```

```{r create-ms-figures, eval=FALSE, echo=TRUE, fig.width = 7.1, fig.cap="**Fig. S12**. Number of realized combinatons and compositional diversity of the three simulated grid data sets. Red, blue and green represent the grid.random, grid.patchy.n.ISC and grid.patchy.associated data sets."}
...
```{r perform-randomizations, eval=TRUE, echo=FALSE, fig.width = 7.1, fig.cap="**Fig. S11**. Number of realised combinatons and compositional diversity of the three simulated grid data sets. Red, blue and green represent the grid_random, grid_patchy_n_isc and grid_patchy_associated data sets."}
...
points(y = grid_patchy_associated$Y, x = grid_patchy_associated$X,
       pch = c(22, 17, 18, 2, 20, 18)[grid_patchy_associated$Species],
       col = col_a[grid_patchy_associated$Species],
       bg = c("yellow2")[grid_patchy_associated[grid_patchy_associated[, 3]
                                               == "Sp1", "Species"]],
       cex = c(0.8, 0.8, 0.8, 0.6, 0.8, 1)[grid_patchy_associated$Species],
       lwd = 2)
...
points(y = grid_patchy_n_isc$Y, x = grid_patchy_n_isc$X,
       pch = c(22, 17, 18, 2, 20, 18)[grid_patchy_n_isc$Species],
       col = col_a[grid_patchy_n_isc$Species],
       bg = c("yellow2")[grid_patchy_n_isc[grid_patchy_n_isc[, 3]
                                           == "Sp1", "Species"]],
       cex = c(0.8, 0.8, 0.8, 0.6, 0.8, 1)[grid_patchy_n_isc$Species],
       lwd = 2)
...
```{r knitr-table-null-models, eval=TRUE}
knitr::kable(assoc_csr$`Summary statistics`[["CD"]][, 1:10], format = "html",
...
```{r transect-visualization, eval=TRUE, echo=FALSE, fig.width = 7.1, fig.height = 7.1, fig.cap="**Fig. S17**. Number (number of realised combinations; NRC) and diversity (compositional diversity; CD) of species combinations in real transect data sets along increasing sample unit lengths. Transect data sets presented in the figure include: 'frequent species' (A, C) and 'Plant Functional Types' (B, D). The black line represents the observed value. The blue line and red lines represent the 95% confidence intervals generated by the complete spatial randomization and random shift null models using 999 randomizations."}
# Case study 2: Real transect data sets
...
Bartha, S. and M. Kert&eacute;sz. 1998. The importance of neutral-models in detecting interspecific spatial associations from 'trainsect' data. *Tiscia* 31, 85&ndash;98.

Bartha, S., Cz&aacute;r&aacute;n, T., & Oborny, B. (1995). Spatial constraints masking community assembly rules: a simulation study. *Folia geobotanica*, 30, 471&ndash;482. \doi{10.1007/BF02803977}

Bartha, S., Campetella, G., Ruprecht, E., Kun, A., H&aacute;zi, J., Horv&aacute;th, A., Vir&aacute;gh, K., & Moln&aacute;r, Zs. (2008). Will interannual variability in sand grassland communities increase with climate change? *Community Ecology*, 9, 13&ndash;21. \url{https://www.jstor.org/stable/24113514}

Cz&aacute;r&aacute;n, T. 1993. PATPRO: A Monte-Carlo simulation program for multispecies neighborhood competition. *Abstracta Botanica*, 17, 275&ndash;281. \url{https://www.jstor.org/stable/43519455}
...
Juh&aacute;sz-Nagy, P. (1967). On association among plant populations I. *Acta Biologica Debrecina*, *5*, 43&ndash;56.

Juh&aacute;sz-Nagy, P. (1976). Spatial dependence of plant populations. Part 1. Equivalence analysis (an outline for a new model). *Acta Botanica Academiae Scientiarum Hungaricae*, *22*, 61&ndash;78.

Juh&aacute;sz-Nagy, P. (1984a). Notes on diversity. Part I. Introduction. *Abstracta Botanica*, *8*, 43&ndash;55.

Juh&aacute;sz-Nagy, P. (1984b). Spatial dependence of plant populations. Part 2. A family of new models. *Acta Botanica Hungarica*, *30*, 363&ndash;402.

Juh&aacute;sz-Nagy, P. (1993). Notes on compositional diversity. *Hydrobiologia*, *249*, 173--182.

Juh&aacute;sz-Nagy, P., & Podani, J. (1983). Information theory methods for the study of spatial processes and succession. *Vegetatio*, *51*, 129&ndash;140.

Podani, J. (1984). Analysis of mapped and simulated vegetation patterns by means of computerized sampling techniques *Acta Botanica Hungarica*, 30, 403&ndash;425.

Podani, J., Cz&aacute;r&aacute;n, T., & Bartha, S (1993). Pattern, area and diversity: the importance of spatial scale in species assemblages. *Abstracta Botanica*, 17, 37&ndash;51. \url{https://www.jstor.org/stable/43519429}

Rejm&aacute;nek, M., & Lep&scaron;, J. (1966). Negative associations can reveal interspecific competition and reversal of competitive hierarchies during succession. *Oikos*, 76, 161&ndash;168. \doi{10.2307/3545758}

Tsakalos, J.L., Chelli, S., Campetella, G., Canullo, R., Simonetti, E., Bartha, S. (2022). comspat: An R package to analyze within community spatial association. \doi{10.1111/ecog.06216}

Wensel, L. C. (1975). The treatment of boundary-line overlap in a forest sampling simulator. *Hilgardia*, 43, 143&ndash;159. \doi{10.3733/hilg.v43n05p143}
